(function(){function m(){n=!0;window.llm.enableNavBlocker();h.innerHTML='<div class="has-text-info"><i class="fas fa-exclamation-triangle" aria-hidden="true"></i> Unsaved Changes</div>'}var d=document.getElementById("form--location"),h=document.getElementById("container--form-message"),k=document.getElementById("location--locationID").value,n=!1,b=""===k,f="true"===document.getElementsByTagName("main")[0].getAttribute("data-is-admin");d.addEventListener("submit",function(a){a.preventDefault();h.innerHTML=
'Saving... <i class="fas fa-circle-notch fa-spin" aria-hidden="true"></i>';window.fetch(b?"/locations/doCreate":"/locations/doUpdate",{method:"POST",credentials:"include",body:new URLSearchParams(new FormData(d))}).then(function(a){return a.json()}).then(function(a){a.success&&(n=!1,window.llm.disableNavBlocker());a.success&&b?window.location.href="/locations/"+a.locationID+"/edit":(h.innerHTML="",window.llm.alertModal(a.message,"","OK",a.success?"success":"danger"))})});if(!b){var p=function(){window.fetch("/locations/doDelete",
...
a;l.removeAttribute("disabled");r()})},onshown:function(a,b){g=b}})}})}for(var f=d.getElementsByClassName("input"),g=0;g<f.length;g+=1)f[g].addEventListener("change",m)})();
