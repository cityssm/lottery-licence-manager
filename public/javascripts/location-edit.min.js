...
{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetLocationID:k,sourceLocationID:b})}).then(function(a){return a.json()}).then(function(a){a.success?window.location.reload(!0):llm.alertModal("Merge Not Completed","Please try again.","OK","danger")})},p=function(a){a.preventDefault();b=a.currentTarget.getAttribute("data-location-id");a=a.currentTarget.getAttribute("data-location-display-name");g();llm.confirmModal("Confirm Merge","Are you sure you want to update all licences associated with <em>"+
a+", #"+b+"</em> and associate them with <em>"+d+"</em>?","Yes, Complete Merge","warning",m)},r=function(){var a=l.value.trim().toLowerCase().split(" "),q=document.createElement("div");q.className="list is-hoverable";for(var d=0;d<f.length;d+=1){var c=f[d];if(c.locationID!==t){for(var e=!0,b=0;b<a.length;b+=1){var g=a[b];if(-1===c.locationName.toLowerCase().indexOf(g)){e=!1;break}}e&&(e=document.createElement("a"),e.className="list-item",e.setAttribute("data-location-id",c.locationID),e.setAttribute("data-location-display-name",
c.locationDisplayName),e.addEventListener("click",p),e.innerHTML='<div class="level is-marginless">'+('<div class="level-left"><div>'+llm.escapeHTML(c.locationDisplayName)+"<br /><small>"+llm.escapeHTML(c.locationAddress1)+"</small></div></div>")+('<div class="level-right">#'+c.locationID+"</div>")+'</div><div class="has-text-right">'+(0<c.licences_count?' <span class="tag is-info has-tooltip-left" data-tooltip="Licence Count"><span class="icon"><i class="fas fa-certificate" aria-hidden="true"></i></span> <span>'+
