...
1){var e=b[c];if(!p.querySelector("tr[data-ticket-type='"+e.ticketType+"']")){var g=document.createElement("option");g.setAttribute("data-ticket-price",e.ticketPrice.toFixed(2));g.setAttribute("data-ticket-count",e.ticketCount);g.setAttribute("data-prizes-per-deal",e.prizesPerDeal.toFixed(2));g.setAttribute("data-fee-per-unit",(e.feePerUnit||0).toFixed(2));g.value=e.ticketType;g.innerText=e.ticketType+" ("+e.ticketCount+" tickets, $"+e.ticketPrice.toFixed(2)+" each)";d.insertAdjacentElement("beforeend",
g)}}f()}else a(),window.llm.alertModal("No ticket types available","","OK","danger")};window.llm.openHtmlModal("licence-ticketTypeAdd",{onshow:function(a){d=document.getElementById("ticketTypeAdd--ticketType");d.addEventListener("change",f);b=document.getElementById("ticketTypeAdd--unitCount");b.addEventListener("change",e);a.getElementsByTagName("form")[0].addEventListener("submit",c)},onshown:function(b,d){a=d;T(k)}})});e=p.getElementsByClassName("is-amend-ticket-type-unit-count-button");for(f=
0;f<e.length;f+=1)e[f].addEventListener("click",W);e=p.getElementsByClassName("is-delete-ticket-type-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",V);e=p.getElementsByClassName("is-amend-ticket-type-distributor-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",X);e=p.getElementsByClassName("is-amend-ticket-type-manufacturer-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",Y)}if(!n){var F=document.getElementById("is-update-expected-licence-fee-button");F&&
F.addEventListener("click",function(){var a=document.getElementById("licence--licenceFee");a.value=F.getAttribute("data-licence-fee-expected");a.classList.remove("is-danger");a.closest(".field").getElementsByClassName("help")[0].remove();F.remove();c();r=!0});document.getElementById("is-add-transaction-button").addEventListener("click",function(){var a,d=function(b){b&&b.preventDefault();window.fetch("/licences/doAddTransaction",{method:"POST",credentials:"include",body:new URLSearchParams(new FormData(a))}).then(function(a){return a.json()}).then(function(a){a.success&&
window.location.reload(!0)})};window.llm.openHtmlModal("licence-transactionAdd",{onshow:function(b){window.llm.getDefaultConfigProperty("externalReceiptNumber_fieldLabel",function(a){b.querySelector("label[for='transactionAdd--externalReceiptNumber']").innerText=a});document.getElementById("transactionAdd--licenceID").value=k;var c=document.getElementById("licence--licenceFee").value,e=document.getElementById("licence--transactionTotal"),e=e?e.innerText:0;document.getElementById("transactionAdd--licenceFee").innerText=
...
"true";d()}))}})})}n||((e=document.getElementById("is-unissue-licence-button"))?e.addEventListener("click",function(){window.llm.confirmModal("Unissue Licence?","Are you sure you want to unissue this lottery licence?","Yes, Unissue","danger",function(){window.fetch("/licences/doUnissueLicence",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({licenceID:k})}).then(function(a){return a.json()}).then(function(a){a.success&&window.location.reload(!0)})})}):
document.getElementById("is-issue-licence-button").addEventListener("click",function(){var a=function(){window.fetch("/licences/doIssueLicence",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({licenceID:k})}).then(function(a){return a.json()}).then(function(a){a.success&&window.location.reload(!0)})};G?window.llm.alertModal("Unsaved Changes","Please save all unsaved changes before issuing this licence.","OK","warning"):window.llm.confirmModal("Issue Licence?",
"Are you sure you want to issue this lottery licence?","Yes, Issue","success",a)}))})();
