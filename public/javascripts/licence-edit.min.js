...
" ("+e.ticketCount+" tickets, $"+e.ticketPrice.toFixed(2)+" each)";b.insertAdjacentElement("beforeend",f)}}g()}else a(),cityssm.alertModal("No ticket types available","","OK","danger")};cityssm.openHtmlModal("licence-ticketTypeAdd",{onshow:function(a){b=document.getElementById("ticketTypeAdd--ticketType");b.addEventListener("change",g);c=document.getElementById("ticketTypeAdd--unitCount");c.addEventListener("change",e);a.getElementsByTagName("form")[0].addEventListener("submit",d)},onshown:function(b,
c){a=c;U(k)}})});e=p.getElementsByClassName("is-amend-ticket-type-unit-count-button");for(g=0;g<e.length;g+=1)e[g].addEventListener("click",X);e=p.getElementsByClassName("is-delete-ticket-type-button");for(g=0;g<e.length;g+=1)e[g].addEventListener("click",W);e=p.getElementsByClassName("is-amend-ticket-type-distributor-button");for(g=0;g<e.length;g+=1)e[g].addEventListener("click",Y);e=p.getElementsByClassName("is-amend-ticket-type-manufacturer-button");for(g=0;g<e.length;g+=1)e[g].addEventListener("click",
Z)}if(!n){var G=document.getElementById("is-update-expected-licence-fee-button");G&&G.addEventListener("click",function(){var a=document.getElementById("licence--licenceFee");a.value=G.getAttribute("data-licence-fee-expected");a.classList.remove("is-danger");a.closest(".field").getElementsByClassName("help")[0].remove();G.remove();d();r=!0});document.getElementById("is-add-transaction-button").addEventListener("click",function(){var a,b=function(b){b&&b.preventDefault();cityssm.postJSON("/licences/doAddTransaction",
...
"true";b()}))}})});var H=document.getElementById("is-void-transaction-button");H&&H.addEventListener("click",function(){if(C)cityssm.alertModal("Unsaved Changes","Please save all unsaved changes before issuing this licence.","OK","warning");else{var a=(-1*H.getAttribute("data-transaction-amount")).toFixed(2);cityssm.confirmModal("Void Transaction?","<strong>Are you sure you want to void this transaction?</strong><br />If the history of this transaction should be maintained, it may be preferred to create a new transaction for $ "+
a+".","Void Transaction","warning",function(){cityssm.postJSON("/licences/doVoidTransaction",{licenceID:l,transactionIndex:H.getAttribute("data-transaction-index")},function(a){a.success&&window.location.reload(!0)})})}})}n||((e=document.getElementById("is-unissue-licence-button"))?e.addEventListener("click",function(){cityssm.confirmModal("Unissue Licence?","Are you sure you want to unissue this lottery licence?","Yes, Unissue","danger",function(){cityssm.postJSON("/licences/doUnissueLicence",{licenceID:l},
function(a){a.success&&window.location.reload(!0)})})}):(e=function(){var a=function(){cityssm.postJSON("/licences/doIssueLicence",{licenceID:l},function(a){a.success&&window.location.reload(!0)})};C?cityssm.alertModal("Unsaved Changes","Please save all unsaved changes before issuing this licence.","OK","warning"):cityssm.confirmModal("Issue Licence?","Are you sure you want to issue this lottery licence?","Yes, Issue","success",a)},document.getElementById("is-issue-licence-button").addEventListener("click",
e),document.getElementById("is-not-issued-tag").addEventListener("dblclick",e)))})();
