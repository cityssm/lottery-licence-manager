...
t.addEventListener("keyup",y);document.getElementsByClassName("is-organization-lookup-button")[0].addEventListener("click",function(){0===q.length&&window.fetch("/organizations/doGetAll",{method:"get",credentials:"include"}).then(function(a){return a.json()}).then(function(a){q=a;t.removeAttribute("disabled");y()});window.llm.showModal(B)});d=B.getElementsByClassName("is-cancel-button");for(e=0;e<d.length;e+=1)d[e].addEventListener("click",window.llm.hideModal);document.getElementById("licence--municipality").addEventListener("change",
function(a){window.fetch("/licences/doGetDistinctLocations",{method:"POST",credentials:"include",body:JSON.stringify({municipality:a.currentTarget.value}),headers:{"Content-Type":"application/json"}}).then(function(a){return a.json()}).then(function(a){var b=document.getElementById("datalist--licence--location");b.innerHTML="";for(var c=0;c<a.length;c+=1){var d=document.createElement("option");d.value=a[c];b.insertAdjacentElement("beforeend",d)}})});var d=document.getElementById("licence--licenceTypeKey"),
