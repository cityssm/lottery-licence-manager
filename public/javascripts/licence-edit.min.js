...
for(var d=A.getElementsByTagName("input"),b=0;b<d.length;b+=1)d[b].setAttribute("min",a)});w.addEventListener("change",function(){for(var a=w.value,d=A.getElementsByTagName("input"),b=0;b<d.length;b+=1)d[b].setAttribute("max",a)});var da=function(a){a.currentTarget.closest(".panel-block").remove();r=!0;c()},S=function(a){var d="";if(a)if(a instanceof Date)d=a.getFullYear()+"-"+("00"+(a.getMonth()+1)).slice(-2)+"-"+("00"+a.getDate()).slice(-2);else if(a.constructor===String)d=a;else if(a instanceof
...
b=d.querySelector("input[name='ticketType_licenceFee']");b.value=a;b.nextElementSibling.innerText="$ "+a;g();E();c();r=!0},h=function(){document.getElementById("amendUnit_licenceFee").value=(e.feePerUnit*document.getElementById("amendUnit_unitCount").value).toFixed(2)};window.llm.openHtmlModal("licence-ticketTypeUnitAmend",{onshow:function(a){document.getElementById("amendUnit_ticketType").value=b;var c=d.querySelector("input[name='ticketType_unitCount']").value;document.getElementById("amendUnit_unitCountCurrent").value=
c;var g=document.getElementById("amendUnit_unitCount");g.value=c;T(function(a){e=a.find(function(a){return a.ticketType===b});g.addEventListener("change",h);h()});a.getElementsByTagName("form")[0].addEventListener("submit",f)},onshown:function(a,b){g=b}})},X=function(a){var d,b=a.currentTarget.closest("td").previousElementSibling,e=function(a){b.getElementsByTagName("input")[0].value=a.currentTarget.getAttribute("data-location-id");b.getElementsByTagName("span")[0].innerText=a.currentTarget.getAttribute("data-location-display-name");
...
c)}}b=document.getElementById("container--distributorLookup");window.llm.clearElement(b);b.insertAdjacentElement("beforeend",a)})},onshown:function(a,b){d=b}})},Y=function(a){var d,b=a.currentTarget.closest("td").previousElementSibling,e=function(a){b.getElementsByTagName("input")[0].value=a.currentTarget.getAttribute("data-location-id");b.getElementsByTagName("span")[0].innerText=a.currentTarget.getAttribute("data-location-display-name");d();c()};window.llm.openHtmlModal("licence-manufacturerLookup",
...
window.llm.clearElement(b);b.insertAdjacentElement("beforeend",a)})},onshown:function(a,b){d=b}})};U=function(a){var d=a.ticketType,b=document.createElement("tr");b.setAttribute("data-ticket-type",d);b.insertAdjacentHTML("beforeend",'<td><input name="ticketType_ticketType" type="hidden" value="'+d+'" /><span>'+d+"</span></td>");d=a.unitCount;b.insertAdjacentHTML("beforeend",'<td class="has-text-right"><input name="ticketType_unitCount" type="hidden" value="'+d+'" /><span>'+d+"</span></td>");var e=
...
d){a=d;T(k)}})});e=p.getElementsByClassName("is-amend-ticket-type-unit-count-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",W);e=p.getElementsByClassName("is-delete-ticket-type-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",V);e=p.getElementsByClassName("is-amend-ticket-type-distributor-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",X);e=p.getElementsByClassName("is-amend-ticket-type-manufacturer-button");for(f=0;f<e.length;f+=1)e[f].addEventListener("click",
Y)}if(!n){var F=document.getElementById("is-update-expected-licence-fee-button");F&&F.addEventListener("click",function(){var a=document.getElementById("licence--licenceFee");a.value=F.getAttribute("data-licence-fee-expected");a.classList.remove("is-danger");a.closest(".field").getElementsByClassName("help")[0].remove();F.remove();c();r=!0});document.getElementById("is-add-transaction-button").addEventListener("click",function(){var a,d=function(b){b&&b.preventDefault();window.fetch("/licences/doAddTransaction",
...
e=document.getElementById("licence--transactionTotal"),e=e?e.innerText:0;document.getElementById("transactionAdd--licenceFee").innerText=c;document.getElementById("transactionAdd--transactionTotal").innerText=e;c=(c-e).toFixed(2);document.getElementById("transactionAdd--discrepancy").innerText=c;document.getElementById("transactionAdd--transactionAmount").value=c;a=b.getElementsByTagName("form")[0];a.addEventListener("submit",d);Z||(c=document.getElementById("is-add-transaction-issue-licence-button"),
c.classList.remove("is-hidden"),c.addEventListener("click",function(){document.getElementById("transactionAdd--issueLicence").value="true";d()}))}})})}n||((e=document.getElementById("is-unissue-licence-button"))?e.addEventListener("click",function(){window.llm.confirmModal("Unissue Licence?","Are you sure you want to unissue this lottery licence?","Yes, Unissue","danger",function(){window.fetch("/licences/doUnissueLicence",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},
body:JSON.stringify({licenceID:k})}).then(function(a){return a.json()}).then(function(a){a.success&&window.location.reload(!0)})})}):document.getElementById("is-issue-licence-button").addEventListener("click",function(){var a=function(){window.fetch("/licences/doIssueLicence",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({licenceID:k})}).then(function(a){return a.json()}).then(function(a){a.success&&window.location.reload(!0)})};G?window.llm.alertModal("Unsaved Changes",
"Please save all unsaved changes before issuing this licence.","OK","warning"):window.llm.confirmModal("Issue Licence?","Are you sure you want to issue this lottery licence?","Yes, Issue","success",a)}))})();
