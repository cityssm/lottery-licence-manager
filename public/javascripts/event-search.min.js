...
b.eventDate+'">'+window.llm.escapeHTML(b.externalLicenceNumber)+"<br /><small>Licence #"+b.licenceID+"</small></a></div>")+('<div class="column">'+window.llm.escapeHTML(b.organizationName)+"</div>")+('<div class="column">'+g+"<br /><small>"+window.llm.escapeHTML(b.licenceDetails)+"</small></div>")+('<div class="column">'+window.llm.escapeHTML(b.locationDisplayName)+"<br /><small>"+b.startTimeString+" to "+b.endTimeString+"</small></div>")+(b.canUpdate?'<div class="column is-narrow is-hidden-print"><a class="button is-small" href="/events/'+
b.licenceID+"/"+b.eventDate+'/edit"><span class="icon"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span><span>Edit</span></a></div>':"")+"</div></li>")}e.insertAdjacentElement("beforeend",f)}})}var l=window.llm.arrayToObject(config_licenceTypes,"licenceTypeKey"),a=document.getElementById("filter--month"),c=document.getElementById("filter--year"),e=document.getElementById("container--events");document.getElementById("filter--previous").addEventListener("click",function(){"1"===a.value?(a.value=
"12",c.value=parseInt(c.value)-1):a.value=parseInt(a.value)-1;d()});document.getElementById("filter--next").addEventListener("click",function(){"12"===a.value?(a.value="1",c.value=parseInt(c.value)+1):a.value=parseInt(a.value)+1;d()});a.addEventListener("change",d);c.addEventListener("change",d);d()})();
