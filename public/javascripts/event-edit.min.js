(function(){function n(){cityssm.enableNavBlocker();d&&d.setAttribute("disabled","disabled");e.innerHTML='<span class="tag is-light is-info is-medium"><span class="icon"><i class="fas fa-exclamation-triangle" aria-hidden="true"></i></span> <span>Unsaved Changes</span></div>'}function c(){var a=((p.value||0)-(q.value||0)-(r.value||0)).toFixed(2);t.value=a;u.setAttribute("max",a)}var d=document.getElementById("eventNav--eventDate"),k=document.getElementById("form--event"),e=document.getElementById("container--form-message"),
l=document.getElementById("event--licenceID").value,v=document.getElementById("event--eventDate").value;k.addEventListener("submit",function(a){a.preventDefault();e.innerHTML='Saving... <i class="fas fa-circle-notch fa-spin" aria-hidden="true"></i>';cityssm.postJSON("/events/doSave",k,function(a){a.success&&(cityssm.disableNavBlocker(),d.removeAttribute("disabled"));e.innerHTML="";cityssm.alertModal(a.message,"","OK",a.success?"success":"danger")})});document.getElementById("is-delete-event-button").addEventListener("click",
function(a){a.preventDefault();cityssm.confirmModal("Delete Event?","Are you sure you want to delete this event?","Yes, Delete","danger",function(){cityssm.postJSON("/events/doDelete",{licenceID:l,eventDate:v},function(a){a.success&&(cityssm.disableNavBlocker(),window.location.href="/licences/"+l)})})});for(var m=k.querySelectorAll("input, select, textarea"),b=0;b<m.length;b+=1)""!==m[b].name&&m[b].addEventListener("change",n);document.getElementById("is-bank-information-lookup-button").addEventListener("click",
