...
config.licences.feeCalculationFn = (licenceObject) => {
    ...
    if (licenceObject.licenceTypeKey === 'BI') {
        ...
        if (licenceFieldData.discount1_tickets !== '' &&
            licenceFieldData.discount1_cost !== '') {
            const discountTicketCost = Number.parseFloat(licenceFieldData.discount1_cost) /
                Number.parseInt(licenceFieldData.discount1_tickets, 10);
            ticketCost = Math.min(ticketCost, discountTicketCost);
        }
        if (licenceFieldData.discount2_tickets !== '' &&
            licenceFieldData.discount2_cost !== '') {
            const discountTicketCost = Number.parseFloat(licenceFieldData.discount2_cost) /
                Number.parseInt(licenceFieldData.discount2_tickets, 10);
            ticketCost = Math.min(ticketCost, discountTicketCost);
        }
        if (licenceFieldData.discount3_tickets !== '' &&
            licenceFieldData.discount3_cost !== '') {
            const discountTicketCost = Number.parseFloat(licenceFieldData.discount3_cost) /
