...
config.licences.feeCalculationFn = (
  ...
  if (licenceObject.licenceTypeKey === 'BI') {
    ...
    message =
      '3% of $' +
      licenceObject.totalPrizeValue.toFixed(2) +
      ' times ' +
      licenceObject.events.length.toString() +
      ' events'
  } else if (licenceObject.licenceTypeKey === 'RA') {
    const licenceFieldData = objectFunctions.fieldDataArrayToObject(
      licenceObject.licenceFields
    )
    ...
    if (
      licenceFieldData.discount1_tickets !== '' &&
      licenceFieldData.discount1_cost !== ''
    ) {
      const discountTicketCost =
        Number.parseFloat(licenceFieldData.discount1_cost) /
        Number.parseInt(licenceFieldData.discount1_tickets, 10)

      ticketCost = Math.min(ticketCost, discountTicketCost)
    }

    if (
      licenceFieldData.discount2_tickets !== '' &&
      licenceFieldData.discount2_cost !== ''
    ) {
      const discountTicketCost =
        Number.parseFloat(licenceFieldData.discount2_cost) /
        Number.parseInt(licenceFieldData.discount2_tickets, 10)

      ticketCost = Math.min(ticketCost, discountTicketCost)
    }

    if (
      licenceFieldData.discount3_tickets !== '' &&
      licenceFieldData.discount3_cost !== ''
    ) {
      const discountTicketCost =
        Number.parseFloat(licenceFieldData.discount3_cost) /
        Number.parseInt(licenceFieldData.discount3_tickets, 10)
      ticketCost = Math.min(ticketCost, discountTicketCost)
    }
    ...
    if (totalPrizeValue < minPrizeValue) {
      ...
      message =
        'Total Prize Value must be a minimum of $' +
        minPrizeValue.toFixed(2) +
        '.'
    }
  }
...
const licenceTypeNevada = config.licenceTypes.find(
  (licenceType) => licenceType.licenceTypeKey === 'NV'
)
...
for (const nevadaTicketType of licenceTypeNevada.ticketTypes) {
  nevadaTicketType.feePerUnit =
    Math.round(nevadaTicketType.prizesPerDeal * 0.03 * 100) / 100
}
