...
config.reminderCategories = [
  {
    ...
    categoryDescription:
      "To be submitted at the beginning on an organization's fiscal year.",
    isActive: true,
    reminderTypes: [
      {
...
config.licences = {
  feeCalculationFn: (licenceObject: recordTypes.LotteryLicence) => {
...
config.licenceTypes = [
  ...
  },
  {
    ...
    printSettings: {
      agco_useLicenceDatesAsEventDates: true,
      agco_additionalLicenceDetailsHTMLFn: (
        licenceObject: recordTypes.LotteryLicence
      ): string => {
        const drawCountField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-drawCount'
        )

        const ticketCountField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-ticketCount'
        )
        const ticketCostField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-ticketCost'
        )

        const discountTicketsField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount1_tickets'
        )
        const discountCostField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount1_cost'
        )

        const discount2TicketsField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount2_tickets'
        )
        const discount2CostField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount2_cost'
        )

        const discount3TicketsField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount3_tickets'
        )
        const discount3CostField = licenceObject.licenceFields.find(
          (field) => field.fieldKey === 'RA-discount3_cost'
        )

        return (
          drawCountField.fieldValue +
          ' draw' +
