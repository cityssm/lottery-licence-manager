...
(() => {
  ...
  const tableElement = document.querySelector("#table--financialSummary") as HTMLTableElement;
  ...
  const getFinancialSummaryFunction = () => {
    ...
    cityssm.postJSON(urlPrefix + "/events/doGetFinancialSummary", formElement, (summary: Array<{
      ...
      for (const licenceTypeSummaryObject of summary) {
        ...
        const trElement: HTMLTableRowElement =
          tbodyElement.querySelector("tr[data-licence-type-key='" + licenceTypeSummaryObject.licenceTypeKey + "']");

        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='licenceCount']") as HTMLSpanElement).textContent =
        ...
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='costs_receiptsSum']") as HTMLSpanElement).textContent =
          "$" + licenceTypeSummaryObject.costs_receiptsSum.toFixed(2);
        costs_receiptsSum += licenceTypeSummaryObject.costs_receiptsSum;

        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='costs_adminSum']") as HTMLSpanElement).textContent =
          "$" + licenceTypeSummaryObject.costs_adminSum.toFixed(2);
        costs_adminSum += licenceTypeSummaryObject.costs_adminSum;

        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='costs_prizesAwardedSum']") as HTMLSpanElement).textContent =
        ...
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='costs_amountDonatedSum']") as HTMLSpanElement).textContent =
        ...
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (trElement.querySelector("[data-field='licenceFeeSum']") as HTMLSpanElement).textContent =
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='licenceCount']") as HTMLSpanElement).textContent = licenceCount.toString();
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='costs_receiptsSum']") as HTMLSpanElement).textContent =
        "$" + costs_receiptsSum.toFixed(2);

      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='costs_adminSum']") as HTMLSpanElement).textContent =
        "$" + costs_adminSum.toFixed(2);

      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='costs_prizesAwardedSum']") as HTMLSpanElement).textContent =
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='costs_amountDonatedSum']") as HTMLSpanElement).textContent =
        "$" + costs_amountDonatedSum.toFixed(2);

      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      (tfootElement.querySelector("[data-field='licenceFeeSum']") as HTMLSpanElement).textContent =
