...
;(() => {
  ...
  formElement.addEventListener('submit', (formEvent) => {
    ...
    const eventDateInputElements =
      eventsContainerElement.querySelectorAll('input')

    if (eventDateInputElements.length === 0) {
      cityssm.alertModal(
        'Event Date Error',
        'Please ensure there is at least one event date.',
        'OK',
        'warning'
      )
      return
    ...
    cityssm.postJSON(
      ...
      (responseJSON: {
        ...
        if (responseJSON.success && isCreate) {
          ...
          const removeInputElements = document.querySelectorAll(
            '.is-removed-after-save'
          )
  ...
  llm.licenceEdit.setUnsavedChangesFunction = (changeEvent?: Event) => {
    ...
    formMessageElement.innerHTML =
      '<span class="tag is-light is-info is-medium">' +
    ...
    if (!isCreate) {
      document
        .querySelector('#is-add-transaction-button')
        .setAttribute('disabled', 'disabled')
      document
        .querySelector('#is-disabled-transaction-message')
        .classList.remove('is-hidden')
    }

    if (changeEvent) {
      const currentTargetType =
        changeEvent.currentTarget instanceof HTMLInputElement
          ? changeEvent.currentTarget.type
          : ''
  ...
  const externalLicenceNumberUnlockButtonElement = document.querySelector(
    '#is-external-licence-number-unlock-button'
  )

  if (externalLicenceNumberUnlockButtonElement) {
    externalLicenceNumberUnlockButtonElement.addEventListener('click', () => {
      const externalLicenceNumberElement = document.querySelector(
...

  {
    ...
    const organizationLookupFunction_setOrganization = (clickEvent: Event) => {
      ...
      ;(
        document.querySelector('#licence--organizationID') as HTMLInputElement
      ).value = organizationElement.getAttribute('data-organization-id')

      ;(
        document.querySelector('#licence--organizationName') as HTMLInputElement
      ).value = organizationElement.getAttribute('data-organization-name')
    ...
    const organizationLookupFunction_refreshResults = () => {
      ...
      for (const organizationObject of organizationList) {
        ...
        const organizationName =
          organizationObject.organizationName.toLowerCase()
        ...
        if (doDisplayRecord) {
          ...
          listItemElement.dataset.organizationId =
            organizationObject.organizationID.toString()
          listItemElement.dataset.organizationName =
            organizationObject.organizationName
          listItemElement.setAttribute('href', '#')
          listItemElement.innerHTML =
            cityssm.escapeHTML(organizationObject.organizationName) +
            '<br />' +
          ...
          listItemElement.addEventListener(
            'click',
            organizationLookupFunction_setOrganization
          )
    ...
    const organizationLookupFunction_openModal = () => {
      cityssm.openHtmlModal('licence-organizationLookup', {
        onshow(): void {
          organizationLookupSearchStringElement = document.querySelector(
            '#organizationLookup--searchStr'
          ) as HTMLInputElement
          organizationLookupSearchStringElement.addEventListener(
            'keyup',
            organizationLookupFunction_refreshResults
          )

          organizationLookupResultsElement = document.querySelector(
            '#container--organizationLookup'
          )

          if (organizationList.length === 0) {
            cityssm.postJSON(
              urlPrefix + '/organizations/doGetAll',
              undefined,
              (organizationListResponse: recordTypes.Organization[]) => {
                organizationList = organizationListResponse
                organizationLookupSearchStringElement.removeAttribute(
                  'disabled'
                )
                organizationLookupFunction_refreshResults()
    ...
    document
      .querySelector('#is-organization-lookup-button')
      .addEventListener('click', organizationLookupFunction_openModal)
    document
      .querySelector('#licence--organizationName')
      .addEventListener('dblclick', organizationLookupFunction_openModal)
  }
  ...
  llm.licenceEdit.loadLocationListFunction = (
    callbackFunction: (locations: recordTypes.Location[]) => void
  ) => {
    if (locationList.length === 0) {
      cityssm.postJSON(
        urlPrefix + '/locations/doGetLocations',
        {},
...

  {
    ...
    const locationLookupFunction_setLocation = (
      locationID: number,
      locationDisplayName: string
    ) => {
      ;(
        document.querySelector('#licence--locationID') as HTMLInputElement
    ...
    const locationLookupFunction_setLocationFromExisting = (
      clickEvent: Event
    ) => {
      clickEvent.preventDefault()
    ...
    const locationLookupFunction_refreshResults = () => {
      ...
      for (const locationObject of locationList) {
        ...
        if (doDisplayRecord) {
          ...
          const listItemElement = document.createElement('a')
          listItemElement.className = 'panel-block is-block'
          listItemElement.dataset.locationId =
            locationObject.locationID.toString()
          listItemElement.dataset.locationDisplayName =
          ...
          listItemElement.innerHTML =
            ...
            '</div>' +
            '<div class="column">' +
            cityssm.escapeHTML(locationObject.locationDisplayName) +
            '</div>' +
          ...
          listItemElement.addEventListener(
            'click',
            locationLookupFunction_setLocationFromExisting
          )
          listElement.append(listItemElement)
        }
      }
    ...
    const locationLookupFunction_openModal = () => {
      cityssm.openHtmlModal('licence-locationLookup', {
        onshow(modalElement) {
          ...
          locationLookup_searchStringElement = document.querySelector(
            '#locationLookup--searchStr'
          ) as HTMLInputElement
          locationLookup_searchStringElement.addEventListener(
            'keyup',
            locationLookupFunction_refreshResults
          )

          locationLookup_resultsElement = document.querySelector(
            '#container--locationLookup'
          )
          ...
          document
            .querySelector('#form--newLocation')
            .addEventListener('submit', (formEvent) => {
              formEvent.preventDefault()

              cityssm.postJSON(
                urlPrefix + '/locations/doCreate',
                ...
                (responseJSON: {
                  ...
                  if (responseJSON.success) {
                    locationList = []
                    locationLookupFunction_setLocation(
                      responseJSON.locationID,
                      responseJSON.locationDisplayName
                    )
                    locationLookup_closeModalFunction()
                  }
                }
    ...
    document
      ...
      .addEventListener('click', locationLookupFunction_openModal)
    document
      .querySelector('#licence--locationDisplayName')
      .addEventListener('dblclick', locationLookupFunction_openModal)
  }
  ...
  document
    .querySelector('#is-endDateString-year-button')
...

  {
    ...
    const termsConditionsLookupModalElement = document.querySelector(
      '#is-termsConditions-lookup-modal'
    ) as HTMLElement
    const termsConditionsLookupResultsElement = document.querySelector(
      '#container--termsConditionsPrevious'
    ) as HTMLElement

    const termsConditionsLookupFunction_setTermsConditions = (
      clickEvent: Event
    ) => {
      clickEvent.preventDefault()

      const termsConditionsIndex = Number.parseInt(
        (clickEvent.currentTarget as HTMLInputElement).getAttribute(
          'data-terms-conditions-index'
        ),
        10
      )

      const termsConditionsElement = document.querySelector(
        '#licence--termsConditions'
      ) as HTMLTextAreaElement
      termsConditionsElement.value =
        termsConditionsList[termsConditionsIndex].termsConditions
    ...
    document
      .querySelector('#is-termsConditions-lookup-button')
      .addEventListener('click', () => {
        termsConditionsList = []
        ...
        termsConditionsLookupResultsElement.innerHTML =
        ...
        cityssm.postJSON(
          urlPrefix + '/licences/doGetDistinctTermsConditions',
          {
            organizationID
          },
          (termsConditionsListResponse: recordTypes.TermsConditionsStat[]) => {
            ...
            if (termsConditionsList.length === 0) {
              ...
              for (const [
                termsConditionsIndex,
                termsConditionsObject
              ] of termsConditionsList.entries()) {
                const listItemElement = document.createElement('a')
                listItemElement.className = 'panel-block is-block'
                listItemElement.dataset.termsConditionsIndex =
                  termsConditionsIndex.toString()
                ...
                listItemElement.innerHTML =
                  ...
                  (termsConditionsObject.termsConditionsCount > 1
                    ? '<span class="tag is-light has-tooltip-left" data-tooltip="Included on Multiple Licences">' +
                      ...
                      ' <span>Used ' +
                      termsConditionsObject.termsConditionsCount.toString() +
                      ' times</span>' +
                      '</span>'
                    : '') +
                  ...
                  ' <span>' +
                  termsConditionsObject.startDateMaxString +
                  '</span>' +
                  '</span>' +
                  '</p>'
                ...
                listItemElement.addEventListener(
                  'click',
                  termsConditionsLookupFunction_setTermsConditions
                )
                listElement.append(listItemElement)
              }
    ...
    const cancelButtonElements =
      termsConditionsLookupModalElement.querySelectorAll(
        '.is-close-modal-button'
      )
  ...
  const licenceType_selectElement = document.querySelector(
    '#licence--licenceTypeKey'
  ) as HTMLSelectElement

  if (isCreate) {
    const licenceType_fieldContainerElements = document.querySelectorAll(
    ...
    const changeFunction_licenceType = () => {
      ...
      document
        .querySelector('#licence--totalPrizeValue')
        .setAttribute('max', totalPrizeValueMax)

      // Ticket types

      const hasTicketTypes = optionElement.dataset.hasTicketTypes === 'true'

      const totalPrizeValueElement = document.querySelector(
        '#licence--totalPrizeValue'
      )

      if (hasTicketTypes) {
        document
          .querySelector('#is-ticket-types-panel')
          .classList.remove('is-hidden')
    ...
    licenceType_selectElement.addEventListener(
      'change',
      changeFunction_licenceType
    )
  }
...

  {
    ...
    document
      .querySelector('#licence--applicationDateString')
    ...
    const eventFunction_add = (eventDate: Date | string | Event) => {
      ...
      if (eventDate) {
        if (eventDate instanceof Date) {
          ...
          try {
            ...
            const sourceElementID = (eventDate.currentTarget as HTMLElement)
              .dataset.source

            eventDateString = (
              document.querySelector('#' + sourceElementID) as HTMLInputElement
            ).value
          } catch {
            // Ignore
          }
      ...
      eventsContainerElement.insertAdjacentHTML(
        ...
        '<div class="panel-block is-block">' +
          ...
          ('<div class="control is-expanded has-icons-left">' +
            ...
            ' value="' +
            cityssm.escapeHTML(eventDateString) +
            '"' +
            ' min="' +
            cityssm.escapeHTML(startDateElement.value) +
            '"' +
            ' max="' +
            cityssm.escapeHTML(endDateElement.value) +
            '"' +
            ' required />' +
      ...
      buttonElements[buttonElements.length - 1].addEventListener(
        'click',
        eventFunction_remove
      )
    ...
    const eventCalculator_modalElement = document.querySelector(
      '#is-event-calculator-modal'
    ) as HTMLElement

    document
      .querySelectorAll('.is-calculate-events-button')[0]
      .addEventListener('click', () => {
        const eventCount = Number.parseInt(
          (document.querySelector('#eventCalc--eventCount') as HTMLInputElement)
            .value,
          10
        )
        const dayInterval = Number.parseInt(
          (
            document.querySelector(
              '#eventCalc--dayInterval'
            ) as HTMLInputElement
          ).value,
          10
        )
        ...
        const endDate = new Date(
          Number.parseInt(dateSplit[0], 10),
          Number.parseInt(dateSplit[1], 10) - 1,
          Number.parseInt(dateSplit[2], 10)
        )
        ...
        for (
          let eventNumber = 0;
          eventNumber < eventCount && eventDate.getTime() <= endDate.getTime();
          eventNumber += 1
        ) {
    ...
    document
      .querySelector('#is-event-calculator-button')
      .addEventListener('click', () => {
        cityssm.showModal(eventCalculator_modalElement)
      })

    const cancelButtonElements = eventCalculator_modalElement.querySelectorAll(
      '.is-close-modal-button'
    )
    ...
    const addEventButtonElements = document.querySelectorAll(
      '.is-add-event-button'
    )
  ...
  if (!isCreate) {
    const updateFeeButtonElement = document.querySelector(
      '#is-update-expected-licence-fee-button'
    )

    if (updateFeeButtonElement) {
      updateFeeButtonElement.addEventListener('click', () => {
        const licenceFeeElement = document.querySelector(
          '#licence--licenceFee'
        ) as HTMLInputElement

        licenceFeeElement.value = updateFeeButtonElement.getAttribute(
          'data-licence-fee-expected'
        )
    ...
    document
      ...
      .addEventListener('click', () => {
        ...
        const addTransactionFunction = (formEvent?: Event) => {
          ...
          cityssm.postJSON(
            urlPrefix + '/licences/doAddTransaction',
        ...
        cityssm.openHtmlModal('licence-transactionAdd', {
          onshow(modalElement) {
            llm.getDefaultConfigProperty(
              'externalReceiptNumber_fieldLabel',
              (fieldLabel: string) => {
                // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
                ;(
                  modalElement.querySelector(
                    "label[for='transactionAdd--externalReceiptNumber']"
                  ) as HTMLLabelElement
            ...
            const licenceFee = Number.parseFloat(
              (
                document.querySelector(
                  '#licence--licenceFee'
                ) as HTMLInputElement
            ...
            const transactionTotalElement = document.querySelector(
              '#licence--transactionTotal'
            )
            const transactionTotal = Number.parseFloat(
              transactionTotalElement
                ? transactionTotalElement.textContent
                : '0'
            )

            document.querySelector('#transactionAdd--licenceFee').textContent =
              licenceFee.toFixed(2)
            document.querySelector(
              '#transactionAdd--transactionTotal'
            ).textContent = transactionTotal.toFixed(2)

            const discrepancy = (licenceFee - transactionTotal).toFixed(2)

            document.querySelector('#transactionAdd--discrepancy').textContent =
              discrepancy
            ;(
              document.querySelector(
            ...
            addTransactionFormElement.addEventListener(
              'submit',
              addTransactionFunction
            )

            if (!isIssued) {
              const addAndIssueButtonElement = document.querySelector(
                '#is-add-transaction-issue-licence-button'
              )
    ...
    const voidTransactionButtonElement = document.querySelector(
      '#is-void-transaction-button'
    ) as HTMLButtonElement
    ...
    if (voidTransactionButtonElement) {
      voidTransactionButtonElement.addEventListener('click', () => {
        ...
        const voidFunction = () => {
          cityssm.postJSON(
            urlPrefix + '/licences/doVoidTransaction',
            {
              licenceID,
              transactionIndex:
                voidTransactionButtonElement.dataset.transactionIndex
            },
        ...
        const reverseTransactionAmount = (
          Number.parseFloat(
            voidTransactionButtonElement.dataset.transactionAmount
          ) * -1
        ).toFixed(2)
        ...
        cityssm.confirmModal(
          ...
          '<strong>Are you sure you want to void this transaction?</strong><br />' +
            ...
            ' it may be preferred to create a new transaction for $ ' +
  ...
  if (!isCreate) {
    const unissueLicenceButtonElement = document.querySelector(
      '#is-unissue-licence-button'
    )
    ...
    if (unissueLicenceButtonElement) {
      unissueLicenceButtonElement.addEventListener('click', () => {
        const unissueFunction = () => {
          cityssm.postJSON(
            urlPrefix + '/licences/doUnissueLicence',
            {
              licenceID
            },
            (responseJSON: { success: boolean }) => {
      ...
      document
        .querySelector('#is-issue-licence-button')
        .addEventListener('click', issueLicenceFunction)
      document
        .querySelector('#is-not-issued-tag')
        .addEventListener('dblclick', issueLicenceFunction)
    }
  }
})()
