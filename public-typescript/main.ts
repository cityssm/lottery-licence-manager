...
;(() => {
   ...
   */
  function arrayToObject(array, objectKey): Record<string | number, unknown> {
    const object = {}
  ...
  function formatDollarsAsHTML(dollarAmt): string {
    return dollarAmt < 0
  ...
  function initializeDateRangeSelector(
    ...
    const yearOptgroupElement = rangeSelectElement?.querySelector(
      "[data-range-type='year']"
    ) as HTMLOptGroupElement

    const quarterOptgroupElement = rangeSelectElement?.querySelector(
      "[data-range-type='quarter']"
    ) as HTMLOptGroupElement

    const monthOptgroupElement = rangeSelectElement?.querySelector(
      "[data-range-type='month']"
    ) as HTMLOptGroupElement
  ...
  function getDefaultConfigProperty(
    propertyName,
    ...
    try {
      ...
      if (defaultConfigPropertiesString) {
        const defaultConfigProperties = JSON.parse(
          defaultConfigPropertiesString
        )
  ...
  function initializeTabs(tabsListElement, callbackFunctions): void {
    if (!tabsListElement) {
      return
    }
    ...
    const tabClickFunction = (clickEvent: Event) => {
      ...
      const allTabContentElements =
        selectedTabContentElement.parentElement?.querySelectorAll(
          '.tab-content'
        )
