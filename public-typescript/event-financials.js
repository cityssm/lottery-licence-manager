...
(() => {
    ...
    const formatDollarsAsHTMLFunction = (dollarAmt) => {
        if (dollarAmt < 0) {
            return ('<span class="has-text-danger">($' +
    ...
    const getFinancialSummaryFunction = () => {
        ...
        cityssm.postJSON(urlPrefix + '/events/doGetFinancialSummary', formElement, (summary) => {
            ...
            for (const licenceTypeSummaryObject of summary) {
                const trElement = tbodyElement.querySelector("tr[data-licence-type-key='" +
                    licenceTypeSummaryObject.licenceTypeKey +
                    "']");
                trElement.querySelector("[data-field='licenceCount']").textContent = licenceTypeSummaryObject.licenceCount.toString();
                ...
                trElement.querySelector("[data-field='costs_netProceedsSum']").innerHTML = formatDollarsAsHTMLFunction(licenceTypeSummaryObject.costs_netProceedsSum);
            ...
            tfootElement.querySelector("[data-field='costs_receiptsSum']").textContent = '$' + costs_receiptsSum.toFixed(2);
            tfootElement.querySelector("[data-field='costs_adminSum']").textContent = '$' + costs_adminSum.toFixed(2);
            tfootElement.querySelector("[data-field='costs_prizesAwardedSum']").textContent = '$' + costs_prizesAwardedSum.toFixed(2);
            tfootElement.querySelector("[data-field='costs_netProceedsSum']").innerHTML = formatDollarsAsHTMLFunction(costs_netProceedsSum);
            tfootElement.querySelector("[data-field='costs_amountDonatedSum']").textContent = '$' + costs_amountDonatedSum.toFixed(2);
            tfootElement.querySelector("[data-field='licenceFeeSum']").textContent = '$' + licenceFeeSum.toFixed(2);
