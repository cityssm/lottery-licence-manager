...
export const initLicencesDB = (): boolean => {
  ...
  if (!row) {
    ...
    for (let month = 1; month <= 12; month += 1) {
      licencesDB
        .prepare(
          'insert into BankingMonths (bankingMonth) values (' + month + ')'
        )
        .run()
    }
    ...
    licencesDB
      .prepare(
        'create index if not exists LotteryLicences_ExternalLicenceNumberInteger_Index' +
