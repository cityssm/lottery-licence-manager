...
export const parseTicketTypeKey = (
  unparsedTicketTypeKey: string
): ParseTicketTypeKeyReturn => {
...
export default function updateLicence(
  ...
  let externalLicenceNumberInteger = Number.parseInt(
  ...
  // Update licence
  const startDate_now = dateTimeFns.dateStringToInteger(
    requestBody.startDateString
  )
  const endDate_now = dateTimeFns.dateStringToInteger(requestBody.endDateString)
  const startTime_now = dateTimeFns.timeStringToInteger(
  ...
  if (pastLicenceObject.trackUpdatesAsAmendments) {
    if (
      configFunctions.getProperty('amendments.trackDateTimeUpdate') &&
    ...
    if (
      pastLicenceObject.organizationID !==
  ...
  if (ticketTypeIndexes_toDelete) {
    for (const ticketTypeIndex_toDelete of ticketTypeIndexes_toDelete) {
      ...
      if (
        pastLicenceObject.trackUpdatesAsAmendments &&
  ...
  if (typeof requestBody.ticketType_ticketType === 'string') {
    const newTicketTypeIndex =
      getMaxLicenceTicketTypeIndexWithDB(database, requestBody.licenceID) + 1
