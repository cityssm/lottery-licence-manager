...
export default function getInactiveLocations(
  ...
  const rows = database
    .prepare(
      `select
          ...
          d.distributor_endDateMax, m.manufacturer_endDateMax
        from Locations lo
        left join (
          select l.locationID, max(l.endDate) as licences_endDateMax
    ...
    .all(cutoffDateInteger) as Location[]
  ...
  for (const locationObject of rows) {
    ...
    locationObject.recordUpdate_dateString = dateTimeFns.dateToString(
      new Date(locationObject.recordUpdate_timeMillis)
    )

    locationObject.licences_endDateMaxString = dateTimeFns.dateIntegerToString(
      locationObject.licences_endDateMax || 0
    )
    locationObject.distributor_endDateMaxString =
      dateTimeFns.dateIntegerToString(
        locationObject.distributor_endDateMax || 0
      )

    locationObject.manufacturer_endDateMaxString =
      dateTimeFns.dateIntegerToString(
        locationObject.manufacturer_endDateMax || 0
      )
  }
