...
export default function getDashboardStats() {
    ...
    const licenceStats = database
        .prepare(`select ifnull(count(licenceID), 0) as licenceCount,
    ...
    const eventStats = database
        .prepare(`select ifnull(sum(case when eventDate = ? then 1 else 0 end), 0) as todayCount,
    ...
    const reminderStats = database
        .prepare(`select ifnull(sum(case when dueDate = ? then 1 else 0 end), 0) as todayCount,
    ...
    if (reminderStats.todayCount > 0 || reminderStats.upcomingCount > 0) {
        reminders = database
            .prepare(`select r.organizationID, o.organizationName,
