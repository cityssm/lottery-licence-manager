...
export default function getOrganizations(
  requestBody: GetOrganizationsFilters,
  requestUser: User,
  includeOptions: {
    limit: number
    offset?: number
  ...
  const sqlParameters: Array<string | number> = [
  ...
  const rows = database.prepare(sql).all(sqlParameters) as Organization[]
