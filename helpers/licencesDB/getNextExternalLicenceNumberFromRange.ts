...
export default function getNextExternalLicenceNumberFromRange(): number {
  ...
  const rangeStartFromConfig = getApplicationSettingWithDB(
    database,
    'licences.externalLicenceNumber.range.start'
  )

  const rangeStart =
    rangeStartFromConfig === '' ? -1 : Number.parseInt(rangeStartFromConfig, 10)

  const rangeEnd = Number.parseInt(
    getApplicationSettingWithDB(
      database,
  ...
  const row = database
    .prepare(
      `select max(externalLicenceNumberInteger) as maxExternalLicenceNumberInteger
