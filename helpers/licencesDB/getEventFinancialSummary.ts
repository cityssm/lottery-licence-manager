...
export default function getEventFinancialSummary(requestBody: {
  ...
  if (
    ...
    sql += ' and e.eventDate >= ?'
    sqlParameters.push(
      dateTimeFns.dateStringToInteger(requestBody.eventDateStartString)
    )
  }
  ...
  if (requestBody.eventDateEndString && requestBody.eventDateEndString !== '') {
    sql += ' and e.eventDate <= ?'
    sqlParameters.push(
      dateTimeFns.dateStringToInteger(requestBody.eventDateEndString)
    )
  }
  ...
  const rows = database
    .prepare(sql)
    .all(sqlParameters) as EventFinancialSummary[]
