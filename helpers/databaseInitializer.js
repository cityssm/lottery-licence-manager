...
export function initLicencesDB() {
    ...
    if (!row) {
        ...
        licencesDB
            .prepare(`create table if not exists Locations (
          ...
          ${recordColumns})`)
            .run();
        licencesDB
            .prepare(`create table if not exists Organizations (
          ...
          ${recordColumns})`)
            .run();
        licencesDB
            .prepare(`create table if not exists OrganizationRepresentatives (
          ...
          foreign key (organizationID) references Organizations (organizationID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists OrganizationRemarks (
          organizationID integer not null,
          ...
          foreign key (organizationID) references Organizations (organizationID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists OrganizationReminders (
          organizationID integer not null,
          ...
          foreign key (organizationID) references Organizations (organizationID)) without rowid`)
            .run();
        licencesDB
            .prepare('create table BankingMonths (bankingMonth integer primary key not null)')
            .run();
        for (let month = 1; month <= 12; month += 1) {
            licencesDB
                .prepare(`insert into BankingMonths (bankingMonth) values (${month})`)
                .run();
        }
        licencesDB
            .prepare(`create table if not exists OrganizationBankRecords (
          organizationID integer not null,
          ...
          foreign key (organizationID) references Organizations (organizationID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryLicences (
          ...
          foreign key (locationID) references Locations (locationID))`)
            .run();
        licencesDB
            .prepare(`create index if not exists LotteryLicences_ExternalLicenceNumberInteger_Index on LotteryLicences
          (externalLicenceNumberInteger desc) where externalLicenceNumberInteger <> -1`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryLicenceTransactions (
          ...
          foreign key (licenceID) references LotteryLicences (licenceID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryLicenceAmendments (
          licenceID integer not null,
          amendmentIndex integer not null,
          amendmentDate integer not null,
          amendmentTime integer not null,
          ...
          foreign key (licenceID) references LotteryLicences (licenceID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryEvents (
          licenceID integer not null,
          ...
          foreign key (licenceID) references LotteryLicences (licenceID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryLicenceTicketTypes (
          licenceID integer not null,
          ...
          foreign key (manufacturerLocationID) references Locations (locationID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryLicenceFields (
          licenceID integer not null,
          ...
          foreign key (licenceID) references LotteryLicences (licenceID)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryEventCosts (
          licenceID integer not null,
          eventDate integer not null,
          ...
          foreign key (licenceID, eventDate) references LotteryEvents (licenceID, eventDate))`)
            .run();
        licencesDB
            .prepare(`create table if not exists LotteryEventFields (
          licenceID integer not null,
          eventDate integer not null,
          fieldKey varchar(20) not null,
          fieldValue text,
          primary key (licenceID, eventDate, fieldKey),
          foreign key (licenceID, eventDate) references LotteryEvents (licenceID, eventDate)) without rowid`)
            .run();
        licencesDB
            .prepare(`create table if not exists ApplicationSettings (
          ...
          recordUpdate_userName varchar(30) not null,
          recordUpdate_timeMillis integer not null) without rowid`)
            .run();
        ...
        values (?, ?, ?, ?, ?, ?, ?)`;
        licencesDB
            .prepare(settingInsertSQL)
            .run('licences.externalLicenceNumber.range.start', 'External Licence Number: Range Start', `When External Licence Numbers are generated using a range, this value will be used as the minimum for the range.`, '-1', 1, 'init', Date.now());
        licencesDB
            .prepare(settingInsertSQL)
            .run('licences.externalLicenceNumber.range.end', 'External Licence Number: Range End', `When External Licence Numbers are generated using a range, this value will be used as the maximum for the range.`, '0', 2, 'init', Date.now());
        licencesDB.close();
        return true;
    }
    return false;
