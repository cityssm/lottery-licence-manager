...
export function canUpdateObject(object, requestUser) {
    ...
    if (object.recordUpdate_timeMillis +
        configFunctions.getProperty('user.createUpdateWindowMillis') >
        Date.now()) {
        return canUpdate;
    }
...
export function getRawRowsColumns(sql, parameters, userFunctions) {
...
export function resetEventTableStats() {
    eventTableStatsExpiryMillis = -1;
}
export function resetLicenceTableStats() {
...
export function getLicenceTypeSummary(requestBody) {
    ...
    if (requestBody.applicationDateStartString &&
    ...
    if (requestBody.applicationDateEndString &&
...
export function getActiveLicenceSummary(requestBody, requestUser) {
    const database = sqlite(databasePath, {
        readonly: true
    });
...
export function getRecentlyUpdateEvents(requestUser) {
    const database = sqlite(databasePath, {
        readonly: true
    });
    const events = database
        .prepare(`select e.eventDate, e.reportDate,
